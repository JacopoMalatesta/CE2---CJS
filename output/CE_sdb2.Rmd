...
##code and esplanations for CE2 - as much as i could - by myself

# 1 www.beppegrillo.it Inspect the robot.txt and describe what you can and what you should not do. Pay attention to the allow / disallow statements and the definition of user-agent. What do these lines mean?
...
#3 Create a data frame with all the HTML links in the page using the XML::getHTMLLinks(). Then, use a regex to keep only those links that re-direct to other posts of the beppegrillo.it blog (so remove all other links). Finally, achieve the same result using rvest:: instead of XML.
...
#from here i was not able to go much ahead, need more time to revise


# 6 What is crawling?
we should rember that the web is made of connections (by definition); pages that remind to other pages and so on. within each pages there are informations (once they were tags) about the page content and... another relevance index of each page is the numer (weighted by importance) links that converge to it from other web pages.
the web is huge and keeps on growing.
the crawlers are bots that make a big job, moving thru all the links connections trying to count the relervance of each page (on the good side) on the (bad) side they could also take informations from the pages and remind them to the "sender". some crawlers are prorpeitary, some others are open socurce and could be used.
